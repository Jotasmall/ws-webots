...
int listening(struct robot *bot){ //ok-
  int i;
  ...
  if (data[0] == 'U') {
    ...
    writeDecision(0, 0, TRAVELING_CALL, 0, bot);
        ...
        if (codeReceived == ROBOT_LEAVING) {
          ...
          */
          if ((codeReceived >= 100) && (codeReceived <= 107)){	
            ...
            updateEstimations(codeReceived, 0, bot);
          }
        }
        break;
      } /*else {
...
int speaking(int toWhom, int codeTask, int time, int cache, struct robot *bot){ //ok-
